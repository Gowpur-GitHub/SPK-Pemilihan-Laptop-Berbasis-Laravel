                ...
                <form action="{{ route('data-sub-kriteria3.update', $subkriteria->slug) }}" method="POST"
                    ...
                    <div class="mb-3 row">
                        <label for="Processor"
                            ...
                            Kriteria</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('Processor')
                                  is-invalid
                              @enderror" id="Processor"
                                value="{{ old('Processor', $subkriteria->Processor) }}" name="Processor"
                                required>
                            @error('Processor')
                    ...
                    <div class="mb-3 row">
                        <label for="nProcessor"
                            class="col-sm-2 col-form-label text-end fw-semibold text-secondary">Nilai</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control @error('nProcessor')
                                  is-invalid
                              @enderror" id="nProcessor"
                                value="{{ old('nProcessor', $subkriteria->nProcessor) }}"
                                name="nProcessor" required>
                            @error('nProcessor')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
...
<script>
    const Processor = document.querySelector("#Processor");
        ...
        Processor.addEventListener("keyup", () => {
            // let preslug = Processor.value;
